#!/usr/bin/env bash
set -e

DIR=$APACHE_ROOT/openagency
INI=$DIR/openagency.ini
INSTALL=$INI"_INSTALL"

cp $DIR/openagency.wsdl_INSTALL $DIR/openagency.wsdl

if [ ! -f $INI ] ; then
    cp $INSTALL $INI

    while IFS='=' read -r name value ; do
      echo "$name $value"
      sed -i "s/@${name}@/$(echo $value | sed -e 's/\//\\\//g; s/&/\\\&/g')/g" $INI
    done < <(env)

    if [ -n "`grep '@[A-Z_]*@' $INI`" ]
    then
      printf "\nMissed some settings:\n"
      echo "------------------------------"
      grep '@[A-Z_]*@' $INI
      echo "------------------------------"
      printf "\nAdd the missing setting(s) and try again\n\n"
      exit 1
    fi
else

    echo "######  ####### #     # ####### #       ####### ####### ######  ####### ######"
    echo "#     # #       #     # #       #       #       #     # #     # #       #     #"
    echo "#     # #       #     # #       #       #       #     # #     # #       #     #"
    echo "#     # #####   #     # #####   #       #####   #     # ######  #####   ######"
    echo "#     # #        #   #  #       #       #       #     # #       #       #   #"
    echo "#     # #         # #   #       #       #       #     # #       #       #    #"
    echo "######  #######    #    ####### ####### ####### ####### #       ####### #     #"
    echo ""
    echo "#     # ####### ######  #######"
    echo "##   ## #     # #     # #"
    echo "# # # # #     # #     # #"
    echo "#  #  # #     # #     # #####"
    echo "#     # #     # #     # #"
    echo "#     # #     # #     # #"
    echo "#     # ####### ######  #######"

fi

if [ -z "$URL_PATH" ]
then
  printf "\nMissed PATH configuration :\n"
  echo "------------------------------"

  echo "------------------------------"
  printf "\nAdd the missing setting(s) and try again\n\n"
  exit 1
fi

cat - > $APACHE_ROOT/index.html <<EOF
<html>
<head>
<title>openagency $URL_PATH</title>
<meta http-equiv="refresh" content="0; url=${URL_PATH}" />
</head>
<body>
<p><a href="${URL_PATH}/">openagency</a></p>
</body>
</html>
EOF


ln -sf /dev/stdout /var/log/apache2/access.log
ln -sf /dev/stderr /var/log/apache2/error.log

/etc/init.d/memcached start

exec apache2ctl -DFOREGROUND
