...
function runTest() {
  ...
  docker run --rm -e BUILD_NUMBER --network=${COMPOSE_PROJECT_NAME}_default -ti -v "$JUNIT_RESULT_DIR:/output" docker-i.dbc.dk/oa-tester "http://$GOLD_IP:80/gold_oa/" "http://$TEST_IP:80/test_oa/"
  RESULT=$?
